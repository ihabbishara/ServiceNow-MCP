...
- **Proxy:** set `SERVICENOW_PROXY` / `ADO_PROXY` to route each service's HTTPS calls through a corporate HTTP proxy (HTTPS origins are reached via a CONNECT tunnel; proxy credentials in the URL — `http://user:pass@host:port` — are honored). The clients use undici's own `fetch` + `ProxyAgent` from the pinned package (`src/clients/proxy.ts`), so proxying is independent of whichever undici the Node runtime bundles. Per-service and independent — no shared/standard-env fallback, and **no `NO_PROXY` bypass**: each service is all-or-nothing through its own proxy. Unset → direct connection.
