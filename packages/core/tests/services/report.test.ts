...
describe("ReportService", () => {
  it("aggregates counts, majors, change buckets, and actions", async () => {
    const sn = {
      listIncidents: vi
        .fn()
        .mockResolvedValue([inc("INC-P1", "1"), inc("INC-P2", "2"), inc("INC-P2B", "2")]),
      listChangesWithFilters: vi
        .fn()
        .mockResolvedValue([
    ...
    expect(report.failedOrHighRiskChanges.map((c) => c.number)).toEqual([
      "CHG-FAILED",
      "CHG-HIGHRISK"
    ]);
  ...
  it("threads assignmentGroup into every underlying query", async () => {
    ...
    await new ReportService(incidents, sn).generateDailyOpsReport({
      now,
    ...
    expect(listSlaRisks).toHaveBeenCalledWith(
      expect.objectContaining({ assignmentGroup: "Platform SRE" })
    );
    expect(listStaleIncidents).toHaveBeenCalledWith(
      expect.objectContaining({ assignmentGroup: "Platform SRE" })
    );
    expect(listChangesWithFilters).toHaveBeenCalledWith(
      expect.objectContaining({ assignmentGroup: "Platform SRE" })
    );
  });
});
